...
class Swarm1DLinear(Env):
    def __init__(
        ...
        extended_state_space: Tuple[float, float] = (-1.0, 2.0),
    ) -> None:
        ...
        with torch.no_grad():
            ...
            extended_cell_centers_1d = (
                torch.arange(
                    extended_state_space[0] * self.num_intervals, 
                    extended_state_space[1] * self.num_intervals, 
        ...
        if dynamics_ckpt is not None:
            ...
            self.dynamics = UnknownDynamics(
                **dynamics_cfg,
                ...
                extended_state_space=self.extended_state_space,
                device=self.device,
            )
        else:
            self.dynamics = None
    ...
    def get_next_states(self, current_states, actions, mf_transition: bool=False):
        ...
        if mf_transition:
            # We use it for computing next_mu so we need the actual states
            next_states = (
            ...
            next_states += noise
            next_states = torch.clamp(next_states, self.extended_state_space[0], self.extended_state_space[1])
    ...
    def step(
        ...
        if not policy_training:
            with torch.no_grad():
                ...
                self.ra_states = (
                    torch.remainder(ra_next_states, self.state_space[1] - self.state_space[0])
                    + self.state_space[0]
                )
        ...
        if self.exec_type == "eval":
            next_mu = self._step_deterministic(self.ra_states, current_mu)
        else:
